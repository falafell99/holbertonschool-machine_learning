...
class Yolo:
        ...
        for image in images:
            ...
            if hasattr(Image, 'Resampling'):
                resized_pil = pil_img.resize(
                    (input_w, input_h), resample=Image.Resampling.BICUBIC)
            else:
                resized_pil = pil_img.resize(
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """Displays the image with all boundary boxes and info."""
        for i in range(len(boxes)):
            x1, y1, x2, y2 = map(int, boxes[i])
            cv2.rectangle(image, (x1, y1), (x2, y2), (255, 0, 0), 2)

            c_name = self.class_names[box_classes[i]]
            score = round(box_scores[i], 2)
            text = "{} {}".format(c_name, score)
    ...
    def predict(self, folder_path):
        """Predicts and shows bounding boxes for all images in a folder."""
